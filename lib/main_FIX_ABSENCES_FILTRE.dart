...
class FirebaseService {
  ...
  static Future<({int ecoles, int eleves, int abonnes, int encaisse, int impayes, int total})>
    ...
    Query<Map<String,dynamic>> abosQ  = _db.collection('abonnements');
    if (ecoleId != null) {
      usersQ = usersQ.where('ecoleId', isEqualTo: ecoleId);
      abosQ  = abosQ.where('ecoleId', isEqualTo: ecoleId);
    }
    final usersSnap = await usersQ.get();
    final abosSnap  = await abosQ.get();
    final ecoles = ecoleId == null
        ? (await _db.collection('ecoles').get()).docs.length
        : 1;
    final eleves = usersSnap.docs.where((d)=> (d.data())['role']=='eleve').length;
    var abonnes = 0, encaisse = 0, impayes = 0;
    for (final d in abosSnap.docs) {
      final m = d.data();
      final st = statutAbo(m);
      if (st.aJour) { abonnes++; } else { impayes++; }
      if (m['type']=='paye') encaisse += ((m['montant'] as num?)?.toInt() ?? 0);
    }
    return (ecoles: ecoles, eleves: eleves, abonnes: abonnes,
        encaisse: encaisse, impayes: impayes, total: abosSnap.docs.length);
  }
        ...
        if(user.role == UserRole.admin || user.role == UserRole.directeur) ...[
          FutureBuilder<({int ecoles, int eleves, int abonnes, int encaisse, int impayes, int total})>(
            ...
            builder: (ctx, s) {
              ...
              final reverse = d != null ? (d.encaisse * 0.15).round() : 0;
              return GridView.count(crossAxisCount:2, shrinkWrap:true,
                  ...
                  children:[
                    ...
                    StatCard(value:v(d?.abonnes), label:'Parents abonnes', sub:'A jour',
                        icon:Icons.people_rounded, color:AppColors.blue, iconBg:AppColors.blueBg),
                    StatCard(value:s.hasData?'${d!.encaisse}':'...', label:'Encaisse (FCFA)', sub:'Total',
                        icon:Icons.payments_rounded, color:AppColors.green, iconBg:AppColors.greenBg),
                    if (user.role == UserRole.admin)
                      StatCard(value:v(d?.ecoles), label:'Ecoles partenaires', sub:'Actives',
                          icon:Icons.account_balance_rounded, color:AppColors.orange, iconBg:AppColors.orangeBg)
                    else
                      StatCard(value:s.hasData?'$reverse':'...', label:'A reverser (15%)', sub:'Sur encaisse',
                          icon:Icons.volunteer_activism_rounded, color:AppColors.orange, iconBg:AppColors.orangeBg),
                  ]);
            }),
...
class _NotesPageState extends State<NotesPage> {
  ...
  String _filtreType = 'Toutes';   // filtre d'affichage des notes
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children:[
                ...
                Row(children:[
                  ...
                  Expanded(child: DropdownButtonFormField<String>(
                      ...
                      onChanged: (v)=>setState(()=>_filtreType = v ?? 'Toutes'))),
                ]),
                const SizedBox(height:10),
                // ---- Detail des notes ----
                Builder(builder: (context){
                  final docsAffiches = _filtreType=='Toutes'
                      ? docs
                      : docs.where((d)=>(d.data() as Map)['type']==_filtreType).toList();
                  if (docsAffiches.isEmpty) {
                    return SCCard(child: Text('Aucune note de type « $_filtreType ».',
      ...
      child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
        FutureBuilder<({int ecoles, int eleves, int abonnes, int encaisse, int impayes, int total})>(
          ...
          builder: (ctx, s) {
            final d = s.data;
            String v(int? x) => s.hasData ? '${x ?? 0}' : '...';
            final taux = (d != null && d.total > 0) ? ((d.abonnes / d.total) * 100).round() : null;
            return GridView.count(crossAxisCount:2, shrinkWrap:true,
                physics:const NeverScrollableScrollPhysics(),
                crossAxisSpacing:12, mainAxisSpacing:12, childAspectRatio:0.95,
                children:[
                  StatCard(value:s.hasData?'${d!.encaisse}':'...',label:'Encaisse (FCFA)',sub:'Total',icon:Icons.payments_rounded,color:AppColors.green,iconBg:AppColors.greenBg),
                  StatCard(value:v(d?.abonnes),label:'Abonnes actifs',sub:'A jour',icon:Icons.people_rounded,color:AppColors.blue,iconBg:AppColors.blueBg),
                  StatCard(value:v(d?.impayes),label:'Impayes',sub:'Relances actives',icon:Icons.warning_rounded,color:AppColors.orange,iconBg:AppColors.orangeBg),
                  StatCard(value:taux==null?'--':'$taux%',label:'Taux recouvrement',sub:'Abonnes a jour',icon:Icons.check_circle_rounded,color:AppColors.green,iconBg:AppColors.greenBg),
                ]);
          }),
        const SizedBox(height:20),
