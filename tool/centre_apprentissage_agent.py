...
def main():
    ...
    for i in range(0, len(missing), BATCH_SIZE):
        ...
        try:
            ...
            batch_ids = [x["chapter_id"] for x in batch]
            report["errors"].append({"batch": batch_ids, "error": error_message})
            print(f"AGENT: erreur de génération pour {batch_ids} : {error_message}", file=sys.stderr)
            break
